...
core::ResultCode DepthProcessor::processFrames(const cv::Mat& left_frame,
    ...
    try {
        ...
        if (success) {
            ...
            // INVESTOR DEMO FIX: Retrieve and store point cloud from direct disparity conversion
            // This contains ~1M points generated by generatePointCloudFromDisparity()
            result.point_cloud = pimpl_->coreProcessor->getLastGeneratedPointCloud();

            std::cout << "[DepthProcessor] Point cloud retrieved: "
                      << result.point_cloud.points.size() << " points" << std::endl;
