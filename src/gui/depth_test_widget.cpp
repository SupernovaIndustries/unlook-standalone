    ...
    try {
        ...
        if (depth_processor_ && !result.debug_pointcloud_path.empty()) {
            try {
                ...
                qDebug() << "[DepthWidget] Copying point cloud to debug folder...";

                // Copy the pre-generated PLY file to debug directory
                QFile sourceFile(QString::fromStdString(result.debug_pointcloud_path));
                QFile::remove(QString::fromStdString(pointcloud_debug_path));  // Remove if exists
            ...
            qDebug() << "[DepthWidget] No point cloud available to save in debug folder";
        }
