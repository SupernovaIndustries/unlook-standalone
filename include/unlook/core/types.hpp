...
struct DepthResult {
    ...
    // INVESTOR DEMO FIX: Path to exported point cloud file (avoids 1M point copy)
    // Generated by generatePointCloudFromDisparity() and saved to debug PLY file
    // GUI reads from file instead of copying 1M points in memory (memory-safe approach)
    std::string debug_pointcloud_path;
